...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    ...
    alternates: { canonical: locale === 'it' ? `https://www.printsolutionsrl.it/news` : `https://www.printsolutionsrl.it/en/news` },
  };
}
...
function getNews(locale: string): NewsItem[] {
  if (locale === 'it') {
    return [
      { category: "Evento", title: "Print & Digital Days a Pescara: Anteprima Nazionale GreenBox 3", date: "21–23 Maggio 2026", desc: "Print Solution ha partecipato ai Print & Digital Days organizzati da LCA Distribuzione a Santa Teresa di Spoltore (PE). Tre giorni di demo dal vivo con la nuova GreenBox 3 in anteprima nazionale: stampa single-pass CMYK su scatole e shopper con costi ridotti del 40%.", gradient: "from-cyan-500 to-cyan-600", href: "/blog/print-digital-days-pescara-greenbox-3-lca-distribuzione" },
  ...
  return [
    { category: "Event", title: "Print & Digital Days in Pescara: GreenBox 3 National Premiere", date: "21–23 May 2026", desc: "Print Solution attended the Print & Digital Days organised by LCA Distribuzione in Santa Teresa di Spoltore (PE). Three days of live demos featuring the new GreenBox 3 in its Italian national premiere: single-pass CMYK printing on boxes and shoppers with 40% lower print costs.", gradient: "from-cyan-500 to-cyan-600", href: "/blog/print-digital-days-pescara-greenbox-3-lca-distribuzione" },
            ...
            {news.map((n) => (
              <article key={n.title} className="card-modern overflow-hidden hover:-translate-y-0.5 transition-transform duration-300">
                <div className="p-8">
                  ...
                  {n.href && (
                    <Link href={n.href} className="inline-block mt-4 text-cyan-500 text-sm font-semibold hover:underline">
